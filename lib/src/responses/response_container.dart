class ResponseContainer<T> {
  ...
  int get totalPagesCount =>
      responseHeaders != null && responseHeaders!.isNotEmpty
          ? int.tryParse(responseHeaders!['x-wp-totalpages']?.value ?? '0') ?? 0
          : 0;

  int get totalPostsCount =>
      responseHeaders != null && responseHeaders!.isNotEmpty
          ? int.tryParse(responseHeaders!['x-wp-total']?.value ?? '0') ?? 0
          : 0;
